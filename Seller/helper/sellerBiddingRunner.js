...
module.exports = {
  SellerActiveBidding: async (session_token, seller_id) => {
    ...
    var loginUrl =
      ...
      "bidding/seller-active-biddings/" +
      seller_id;
    ...
    if (!newUserUpdate) {
      return {
        status: false,
        message: "Unable to connect to authentication services",
      };
    }
  ...
  SubmitBid: async (inputDic, data) => {
    ...
    console.log(
      `bidding ID here now::::::: response:: ${selectedGroup.bidding_ID}`
    );
  ...
  SellerPendingBiddingItems: async (session_token, seller_id) => {
    ...
    var loginUrl =
      process.env.ORDER_SERVICE_BASE_URL +
      "invoice/pending-seller-invoice/" +
      seller_id;
    ...
    let activities = requests.map((item, index) => ({
      title: `B${String(item.cart.bid.id).padStart(8, "0")}: ${
        item.cart.bid.orderRequest.sparePart.carModel.name
      } ${item.cart.bid.orderRequest.sparePart.name}(${
        item.cart.bid.orderRequest.quantity
      })`,
  ...
  SetItemReadyForDelivery: async (inputDic, data) => {
    ...
    var loginUrl =
      process.env.ORDER_SERVICE_BASE_URL + "invoice/set-item-for-pickup";
    ...
    console.log(
      `bidding ID here now::::::: response:: ${inputDic.selectedBid.activity_type}`
    );
    console.log(newUserUpdate);
