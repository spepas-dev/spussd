...
exports.SELLER_BID_INDEX = asynHandler(async (req, res, next) => {
    ...
    var activities = [];
    let spDetailsRes = await SellerPendingBiddingItems(data.session.token, data.session.external_user.Seller_ID);
...
var itemName  = `${bidCode} . ${item.cart.bid.orderRequest.sparePart.carModel.yearOfMake} ${item.cart.bid.orderRequest.sparePart.carModel.name} ${item.cart.bid.orderRequest.sparePart.name}`
  ...
  let newActivities = [];
   ...
   displayText = displayText.replace('{itemName}',itemName).replace('{unitPrice}',unitPrice).replace('{Quantity}',quantity).replace('{ExpectedDeliveryDate}',expecetedDelivery)
