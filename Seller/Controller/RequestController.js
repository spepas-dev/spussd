...
const { REGISTRATION_STATUS, RESPONSE_CODES,myVars } = require("../../helper/vars");
...
const {SellerActiveBidding, SubmitBid,SellerActiveBiddingGroupedByBrand, SellerActiveBiddingGroupedByPart} = require("../helper/sellerBiddingRunner")
...
exports.SELLER_REQUEST_ALL_INDEX = asynHandler(async (req, res, next) => {
    ...
    var activities = [];
    let spDetailsRes = await SellerActiveBidding(data.session.token, data.session.external_user.Seller_ID);
    ...
    if(total_pages >  inputDic.seller_active_biddings_page_number){
        activities =  UtilityHelper.getPaginatedList(activities,inputDic.seller_active_biddings_page_number);
    }
   ...
   } else if (value == "0")
   {
   ...
   }else if(value == "#")
    {
        if(totalPages >  currentPageNumber){
           ...
           let activities =  UtilityHelper.getPaginatedList(savedBiddingList,inputDic.seller_active_biddings_page_number);


           let displayText = UtilityHelper.generateDisplayText(activities, inputDic.ListTitle);

           console.log(`total pages: ${totalPages} current page: ${inputDic.seller_active_biddings_page_number}`)
...
console.log(`bidding ID here now::::::: ${selectedGroup.default_value.bidding_ID}`)
...
var itemName  = `B${String(orderRequest.id).padStart(8, '0')}`;
  ...
  exports.SELLER_REQUEST_BRAND_INDEX = asynHandler(async (req, res, next) => {
    ...
    var inputDic  = UtilityHelper.formatInputDictionary(data.session.session_input);
    var activities = [];
    let spDetailsRes = await SellerActiveBiddingGroupedByBrand(data.session.token, data.session.external_user.Seller_ID);
    ...
    if(total_pages >  inputDic.seller_brand_page_number){
        activities =  UtilityHelper.getPaginatedList(activities,inputDic.seller_brand_page_number);
    }
   ...
   } else if (value == "0")
   {
   ...
   }else if(value == "#")
    {
        if(totalPages >  currentBrandPageNumber){
           ...
           let activities =  UtilityHelper.getPaginatedList(savedBrandList,inputDic.seller_brand_page_number);


           let displayText = UtilityHelper.generateDisplayText(activities, "Select brand");

           console.log(`total pages: ${totalPages} current page: ${inputDic.seller_brand_page_number}`)
...
exports.SELLER_REQUEST_PART_INDEX = asynHandler(async (req, res, next) => {
    ...
    var inputDic  = UtilityHelper.formatInputDictionary(data.session.session_input);
    var activities = [];
    let spDetailsRes = await SellerActiveBiddingGroupedByPart(data.session.token, data.session.external_user.Seller_ID);
